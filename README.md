...
### 13. SonarQube Implementation (Code Quality & Security)

**Prompt:**
```
Buatkan panduan lengkap implementasi SonarQube untuk CI/CD pipeline dengan detail berikut:

1. Infrastructure Setup:
   - SonarQube deployment di GCP Compute Engine (e2-standard-2, Ubuntu 22.04)
   - PostgreSQL 15 sebagai database backend (Cloud SQL atau on-instance)
   - Nginx reverse proxy dengan SSL (Let's Encrypt)
   - Firewall: allow 443 (HTTPS), restrict 9000 (SonarQube) ke internal only
   - Persistent disk untuk data (50GB SSD)
   - Backup strategy: database dump harian ke GCS

2. SonarQube Configuration:
   - Quality Gate: default strict (coverage >80%, no new blocker/critical issues, duplication <3%)
   - Quality Profiles untuk: Python, JavaScript, TypeScript, Go, Java
   - Security hotspot rules enabled
   - Custom rules: no hardcoded secrets, no TODO in production code
   - Branch analysis dan Pull Request decoration
   - Integration dengan GitHub/GitLab untuk PR comments

3. CI/CD Integration:
   - SonarScanner step di Cloud Build pipeline
   - Break build jika Quality Gate fail
   - Coverage report upload dari unit test
   - Step untuk multi-language project (monorepo)
   - Artifact: simpan scan report ke GCS untuk audit trail

4. Dashboard & Reporting:
   - Project-level dashboard: bugs, vulnerabilities, code smells, technical debt
   - Portfolio dashboard untuk executive summary
   - Weekly email report ke team lead
   - Alert ke Slack channel jika Quality Gate fail

5. Maintenance:
   - Upgrade procedure (zero-downtime)
   - Database maintenance (vacuum, reindex)
   - Log rotation dan cleanup policies
   - Plugin management (sonar-findbugs, sonar-pmd, etc.)

Buatkan juga:
- Terraform module untuk provision SonarQube infrastructure
- Docker-compose untuk local SonarQube development
- sonar-project.properties template untuk berbagai bahasa
- Cloud Build step yaml untuk SonarQube scan
```

**Use Case:** Setup end-to-end code quality gate yang memastikan setiap deploy lolos quality standard sebelum masuk production.
